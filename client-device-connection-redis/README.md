...
| Lua scripting | The `EVAL` command must be permitted for the configured user. When using Redis ACLs, the `@scripting` command category must not be denied. The cache verifies this at startup and fails fast with a descriptive error otherwise. |
